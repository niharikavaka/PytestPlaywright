...
def xtest_forgot_email(set_up_tear_down,email):
    page=set_up_tear_down
    forgot_page=ForgotPage(page)
    success=forgot_page.do_retrieve_password(email)
    expect(success).to_have_text('An e-mail has been sent to you which explains how to reset your password.')


def xtest_forgot_email_with_no_credentials(set_up_tear_down):
    page=set_up_tear_down
    forgot_page=ForgotPage(page)
